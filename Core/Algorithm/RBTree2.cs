...
public partial class RBTree2<TVal, TKey> : IEnumerable<TVal> where TKey : IComparable<TKey> {
   // Constructor --------------------------------------------------------------
   /// <summary>Construct a RBTree, given the key-getter function that provides a key for each value</summary>
   public RBTree2 () => EnsureCapacity (7);
   ...
   /// overwritten
   public void Add (TVal value, TKey key) {
      if (mFree.Count == 0) EnsureCapacity (mA.Length * 2 - 1);
      mRoot = AddImp (mRoot, value, key);
   ...
   int Remove (int h, TKey key) {
      ...
      if (key.CompareTo (node.Key) < 0) {
         ...
         if (key.CompareTo (node.Key) == 0 && node.Right == 0) { mFree.Push (h); return 0; }
