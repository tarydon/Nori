...
public class UndoStack {
   ...
   public void ClubSteps () {
      ...
      for (int i = n; i >= 0; i--) {
         ...
         // In the trivial case where there are NO steps to club, remove the ClubbedStep from
         // the stack and return immediately. 
         if (i == n) { mSteps.RemoveAt (i); break; }
         // Otherwise, move all the steps subsequent to the marker into the ClubbedStep.
         // Note that even if there is only one step to 'Club', we still do it because the ClubbedStep
         // often provides a better description of the operation than the step enclosed inside. A 
         // typical example is the steps auto-generated by a Ledger<T>. Those steps are all functionally
         // fine, but may not have the most meaningful descriptions, and will rely on an enclosing
         // ClubbedStep to provide a more meaningful Undo description to enlighten the user. 
