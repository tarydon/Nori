...
public readonly partial struct OBB {
   unsafe ref struct Builder3 (ReadOnlySpan<Point3f> pts) {
      // The input points
      ...
      public OBB Build () {
         fixed (Point3f* pts = Pts) {
            ...
            var q = stackalloc Point3f[14]; Q = q;
            var r = stackalloc Point3f[5]; R = r;
            DoBuild ();
            return Box;
         }
      }
      ...
      void DoBuild () {
         ...
         var aabb = Box = CreateBox (X, Y, Z); BestScore = Area (aabb.Extent) * 4;
         for (int i = 0; i < 14; i++) Q[i] = P[H[i]];
         ...
         R[0] = P[p0]; R[1] = P[p1]; R[2] = P[p2]; R[3] = P[q0]; R[4] = P[q1];
         for (int i = 0; i < 3; i++)
            for (int j = i + 1; j < 4; j++)
               for (int k = j + 1; k < 5; k++)
                  RefineBox (i, j, k);
      ...
      float ComputeArea (Vector3f u, Vector3f v, Vector3f w) {
         Point3f p = Q[0];
         float uMin, uMax, vMin, vMax, wMin, wMax;
         uMin = uMax = p.X * u.X + p.Y * u.Y + p.Z * u.Z;
         vMin = vMax = p.X * v.X + p.Y * v.Y + p.Z * v.Z;
         wMin = wMax = p.X * w.X + p.Y * w.Y + p.Z * w.Z;
         for (int j = 1; j < 14; j++) {
            p = Q[j];
            var d = p.X * u.X + p.Y * u.Y + p.Z * u.Z;
            if (d < uMin) uMin = d; else if (d > uMax) uMax = d;
            d = p.X * v.X + p.Y * v.Y + p.Z * v.Z;
            if (d < vMin) vMin = d; else if (d > vMax) vMax = d;
            d = p.X * w.X + p.Y * w.Y + p.Z * w.Z;
            if (d < wMin) wMin = d; else if (d > wMax) wMax = d;
         }
         float du = uMax - uMin, dv = vMax - vMin, dw = wMax - wMin;
