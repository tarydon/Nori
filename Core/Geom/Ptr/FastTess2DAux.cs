// ────── ╔╗
// ╔-╦╦-╦╦╬╣ FastTess2DAux.cs
// ║║║║╬║╔╣║ Nested types for the Tessellator class (Nori.Ptr variant)
// ╚╩-╩-╩╝╚╝ ───────────────────────────────────────────────────────────────────────────────────────
using System.Runtime.CompilerServices;
namespace Nori.Pin;

#region class FastTess2D : nested types ------------------------------------------------------------
unsafe partial class FastTess2D {
   // Enumerations ---------------------------------------------------------------------------------
   ...
   readonly struct Segment {
      // Constructors ----------------------------------------------------------
      public Segment (int id, Vertex* vBase, int a, int b, bool diagonal = false) {
         Id = id; Diagonal = diagonal;
         Point2 pa = vBase[a].Pt, pb = vBase[b].Pt;
   ...
   struct Tile {
      ...
      public Tile (int id, Segment* sBase, double yMin, double yMax, int left, int right, int node) {
         (Id, YMin, YMax, Left, Right, Node) = (id, yMin, yMax, left, right, node);
         Hole = sBase[left].PartOnLeft; Check (sBase[left].PartOnLeft != sBase[right].PartOnLeft);
      }
      ...
      public readonly (int B1, int B2) GetBottom (Vertex* vBase) {
         int b1 = 0, b2 = 0;
         if (VBot > 0) {
            ref Vertex v = ref *(vBase + VBot);
            switch (EBot) {
      ...
      public readonly (int T1, int T2) GetTop (Vertex* vBase) {
         int t1 = 0, t2 = 0;
         if (VTop > 0) {
            ref Vertex v = ref *(vBase + VTop);
            switch (ETop) {
      ...
      ref Tile SplitBase (FastTess2D t, ENode kind, int index) {
         ...
         fixed (Node* nBase = t.mN) {
            Node* leaf = nBase + Node;
            Check (leaf->Kind == ENode.Leaf);
            ...
            fixed (Tile* tBase = t.mT) {
               nBase[t.mNN] = new (t.mNN, ENode.Leaf, Id);            // Below
               nBase[t.mNN + 1] = new (t.mNN + 1, ENode.Leaf, nNew); // Above
               Node = leaf->First = t.mNN; leaf->Second = t.mNN + 1;
               t.mNN += 2;

               // Create the new tile by cloning this one
               tBase[nNew] = new Tile (nNew, ref this, leaf->Second);
               return ref *(tBase + nNew);
            }
         }
      }
      ...
      public void SplitY (FastTess2D t, ref Vertex v) {
         ...
         if (VTop != 0) {
            fixed (Vertex* vBase = t.mV) {
               ref Vertex vt = ref *(vBase + VTop);
      ...
      public ref Tile SplitX (FastTess2D t, int segment) {
         ...
         fixed (Segment* sBase = t.mS) fixed (Vertex* vBase = t.mV) {
            #if VERIFY
               double yM = (YMin + YMax) / 2;
               ref Segment S = ref *(sBase + segment);
               ref Segment L = ref *(sBase + Left);
               ref Segment R = ref *(sBase + Right);
               double xL = L.GetX (yM), x = S.GetX (yM), xR = R.GetX (yM);
               Check (xL < x && x < xR);
            #endif
            Right = t1.Left = segment;
            Check (VTop != 0 && VBot != 0);
            ref Segment seg = ref *(sBase + segment); bool diagonal = seg.Diagonal;
            if (!diagonal) { Hole = !seg.PartOnLeft; t1.Hole = seg.PartOnLeft; }
            ref Vertex vt = ref *(vBase + VTop);
            ref Vertex vb = ref *(vBase + VBot);
            switch (ETop) {
               case EChain.HSlice:
