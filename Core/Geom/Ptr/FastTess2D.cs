// ────── ╔╗
// ╔═╦╦═╦╦╬╣ FastTess2D.cs
// ║║║║╬║╔╣║ Implements a Tessellator that can handle non-intersecting simple polygons (Nori.Ptr variant)
// ╚╩═╩═╩╝╚╝ ───────────────────────────────────────────────────────────────────────────────────────
...
public unsafe partial class FastTess2D : IBorrowable<FastTess2D> {
   // Properties ---------------------------------------------------------------
   ...
   public int AddPoly (Poly poly, bool hole) {
      ...
      // the mS array. Both grows happen before the fixed blocks so no pinned arrays
      // are reallocated while pointers are held.
      int n = pts.Length, vStart = mVN;
      Grow (ref mV, mVN, n);
      Grow (ref mS, mSN, n);
      fixed (Vertex* vBase = mV) {
         ...
         for (int i = 0; i < n; i++, mVN++) {
            ...
            vBase[mVN] = new Vertex (mVN, pt, kind);
            mBound += pt; prev = pt; pt = next;
         }

         // vBase remains valid for the segment loop (mV was not grown above)
         fixed (Segment* sBase = mS) {
            for (int i = 0; i < n; i++, mSN++) {
               int j = (i + 1) % n;
               sBase[mSN] = new Segment (mSN, vBase, i + vStart, j + vStart);
            }
         }
      }
      return n;
   ...
   void AddDiagonals () {
      ...
      foreach (var n in mDiagTiles) {
         // Get fresh pointers each iteration since SliceTiles may have grown mT
         fixed (Tile* tBase = mT) fixed (Vertex* vBase = mV) fixed (Segment* sBase = mS) {
            Tile* t0 = tBase + n; if (t0->Id == 0) continue;
            sBase[mSN] = new (mSN, vBase, t0->VTop, t0->VBot, true);
            ref Segment seg = ref *(sBase + mSN); mSN++;
            mLefts.Clear (); mLefts.Add (n);
            int nNewTile = SliceTiles (ref seg);
            // After SliceTiles, mT may have been reallocated. t0 still points into the old pinned
            // array (valid for the duration of this fixed block). t0->VBot and t0->EBot are not
            // modified in the diagonal path, so reading them here is safe.
            fixed (Tile* tBase2 = mT) {
               Tile* t1 = tBase2 + nNewTile;
   ...
   void ExtractTriangles (int n) {
      mStack.Clear ();
      fixed (Tile* tBase = mT) fixed (Vertex* vBase = mV) {
         ref Tile t = ref *(tBase + n);
         if (t.Id == 0) return; else t.Id = 0;
         mStack.Push ((t.VBot, (vBase + t.VBot)->Pt, true));
         (int, Point2, bool) vPrev = (0, Point2.Nil, false);
         for (; ; ) {
            if (t.VTop != 0) {
               Point2 pt = (vBase + t.VTop)->Pt;
            ...
            var (a, b) = t.GetTop (vBase); Check (b == 0);
            t = ref *(tBase + a);
         }
      }
   ...
   int GetAdjacentTile (ref Vertex v, ref Vertex vOther) {
      ...
      if (L == R) return L;
      fixed (Tile* tBase = mT) fixed (Segment* sBase = mS) {
         ref Tile tLeft = ref *(tBase + L);
         ref Segment sRight = ref *(sBase + tLeft.Right);
   ...
   void GatherTiles (int t0, int t1, ref Segment seg) {
      ...
      fixed (Tile* tBase = mT) fixed (Vertex* vBase = mV) {
         while (t0 != t1) {
            ref Tile tile = ref *(tBase + t0);
            var (a, b) = tile.GetBottom (vBase);
            if (a == t1 || b == t1) { mLefts.Add (t1); return; }
            if (b > 0) {
               ref Vertex vb = ref *(vBase + tile.VBot);
   ...
   void InsertBorder () {
      ...
      // AllocTile may grow mT, so call it before entering fixed blocks
      int nNew = AllocTile ();
      fixed (Vertex* vBase = mV) fixed (Segment* sBase = mS) fixed (Node* nBase = mN) fixed (Tile* tBase = mT) {
         // Add the 4 vertices making up the border
         vBase[mVN]     = new (mVN,     p0);
         vBase[mVN + 1] = new (mVN + 1, p1);
         vBase[mVN + 2] = new (mVN + 2, p2);
         vBase[mVN + 3] = new (mVN + 3, p3);
         // Connect them up with two segments (left goes up, right goes down)
         sBase[mSN + 0] = new (mSN + 0, vBase, mVN, mVN + 1);
         sBase[mSN + 1] = new (mSN + 1, vBase, mVN + 3, mVN + 2);
         // Add the root node, and the base tile covering the entire rectangular field
         nBase[0] = new (0, ENode.Leaf, nNew);
         tBase[nNew] = new Tile (nNew, sBase, p0.Y, p1.Y, mSN, mSN + 1, 0);
      }
   ...
   void InsertEndpoints (ref Segment seg) {
      ...
      fixed (Vertex* vBase = mV) {
         ref Vertex v0 = ref *(vBase + seg.A);
         ref Vertex v1 = ref *(vBase + seg.B);
   ...
   ref Tile Locate (Point2 pt) {
      fixed (Node* nBase = mN) fixed (Tile* tBase = mT) fixed (Segment* sBase = mS) fixed (Vertex* vBase = mV) {
         Node* node = nBase, prev = node;
         for (; ; ) {
            bool first;
            switch (node->Kind) {
               case ENode.Y: first = pt.Y < (vBase + node->Index)->Pt.Y; break;
               case ENode.X: first = (sBase + node->Index)->IsLeft (pt); break;
               case ENode.Leaf: return ref *(tBase + node->Index);
               case ENode.Redirect:
            ...
            prev = node;
            node = first ? nBase + node->First : nBase + node->Second;
         }
      }
   }
   ...
   void MergeTiles (ref Tile t0, ref Tile t1) {
      ...
      if (t1.ETop != EChain.Mountain) {
         fixed (Vertex* vBase = mV) {
            ref Vertex vt = ref *(vBase + t0.VTop);
   ...
   int SliceTiles (ref Segment seg) {
      ...
      fixed (Tile* tBase = mT) {
         ...
         for (int i = 0; i < n; i++) {
            ref Tile left = ref *(tBase + mLefts[i]);
            ref Tile right = ref left.SplitX (this, seg.Id);
            mRights.Add (right.Id);
            if (left.VTop == 0) MergeTiles (ref *(tBase + mLefts[i - 1]), ref left);
            if (right.VTop == 0) MergeTiles (ref *(tBase + mRights[i - 1]), ref right);
         }
      }
      return mRights[^1];
   }
