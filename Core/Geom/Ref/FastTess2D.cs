// ────── ╔╗
// ╔═╦╦═╦╦╬╣ FastTess2D.cs
// ║║║║╬║╔╣║ Implements a Tessellator that can handle non-intersecting simple polygons (Nori.Ref variant)
// ╚╩═╩═╩╝╚╝ ───────────────────────────────────────────────────────────────────────────────────────
...
public partial class FastTess2D : IBorrowable<FastTess2D> {
   ...
   public int AddPoly (Poly poly, bool hole) {
      ...
      Grow (ref mV, mVN, n);
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      ...
      for (int i = 0; i < n; i++, mVN++) {
         ...
         Unsafe.Add (ref vBase, mVN) = new Vertex (mVN, pt, kind);
         mBound += pt; prev = pt; pt = next;
      }

      // Now, add the segments corresponding to this newly added contour
      // (mV was not grown above, so vBase remains valid)
      Grow (ref mS, mSN, n);
      ref Segment sBase = ref MemoryMarshal.GetArrayDataReference (mS);
      for (int i = 0; i < n; i++, mSN++) {
         int j = (i + 1) % n;
         Unsafe.Add (ref sBase, mSN) = new Segment (mSN, ref vBase, i + vStart, j + vStart);
      }
      return n;
   }
   ...
   void AddDiagonals () {
      {
         ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
         for (int i = mTN_ - 1; i > 0; i--) {
            ref Tile t = ref Unsafe.Add (ref tBase, i);
      ...
      foreach (var n in mDiagTiles) {
         // Get fresh base refs each iteration - SliceTiles may have grown mT in a prior iteration
         ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
         ref Tile t0 = ref Unsafe.Add (ref tBase, n); if (t0.Id == 0) continue;
         ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
         ref Segment sBase = ref MemoryMarshal.GetArrayDataReference (mS);
         Unsafe.Add (ref sBase, mSN) = new (mSN, ref vBase, t0.VTop, t0.VBot, true);
         ref Segment seg = ref Unsafe.Add (ref sBase, mSN); mSN++;
         mLefts.Clear (); mLefts.Add (n);
         int nNewTile = SliceTiles (ref seg);
         // Re-get tBase after SliceTiles which may have grown mT; t0 may be stale (same as original)
         ref Tile tBase2 = ref MemoryMarshal.GetArrayDataReference (mT);
         ref Tile t1 = ref Unsafe.Add (ref tBase2, nNewTile);
   ...
   void ExtractTriangles (int n) {
      ...
      ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      ref Tile t = ref Unsafe.Add (ref tBase, n);
      if (t.Id == 0) return; else t.Id = 0;
      mStack.Push ((t.VBot, Unsafe.Add (ref vBase, t.VBot).Pt, true));
      (int, Point2, bool) vPrev = (0, Point2.Nil, false);
      for (; ; ) {
         if (t.VTop != 0) {
            Point2 pt = Unsafe.Add (ref vBase, t.VTop).Pt;
         ...
         var (a, b) = t.GetTop (ref vBase); Check (b == 0);
         t = ref Unsafe.Add (ref tBase, a);
      }
   ...
   int GetAdjacentTile (ref Vertex v, ref Vertex vOther) {
      ...
      ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
      ref Segment sBase = ref MemoryMarshal.GetArrayDataReference (mS);
      ref Tile tLeft = ref Unsafe.Add (ref tBase, L);
      ref Segment sRight = ref Unsafe.Add (ref sBase, tLeft.Right);
   ...
   void GatherTiles (int t0, int t1, ref Segment seg) {
      ...
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      while (t0 != t1) {
         ref Tile tile = ref Unsafe.Add (ref tBase, t0);
         var (a, b) = tile.GetBottom (ref vBase);
         if (a == t1 || b == t1) { mLefts.Add (t1); return; }
         if (b > 0) {
            ref Vertex vb = ref Unsafe.Add (ref vBase, tile.VBot);
   ...
   void InsertBorder () {
      ...
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      Unsafe.Add (ref vBase, mVN)     = new (mVN,     p0);
      Unsafe.Add (ref vBase, mVN + 1) = new (mVN + 1, p1);
      Unsafe.Add (ref vBase, mVN + 2) = new (mVN + 2, p2);
      Unsafe.Add (ref vBase, mVN + 3) = new (mVN + 3, p3);
      // Connect them up with two segments (left goes up, right goes down)
      ref Segment sBase = ref MemoryMarshal.GetArrayDataReference (mS);
      Unsafe.Add (ref sBase, mSN + 0) = new (mSN + 0, ref vBase, mVN, mVN + 1);
      Unsafe.Add (ref sBase, mSN + 1) = new (mSN + 1, ref vBase, mVN + 3, mVN + 2);
      // Add the root node, and the base tile covering the entire rectangular field
      // AllocTile may grow mT, so get tBase and nBase after the call
      int nNew = AllocTile ();
      ref Node nBase = ref MemoryMarshal.GetArrayDataReference (mN);
      ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
      Unsafe.Add (ref nBase, 0) = new (0, ENode.Leaf, nNew);
      Unsafe.Add (ref tBase, nNew) = new Tile (nNew, ref sBase, p0.Y, p1.Y, mSN, mSN + 1, 0);
   ...
   void InsertEndpoints (ref Segment seg) {
      ...
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      ref Vertex v0 = ref Unsafe.Add (ref vBase, seg.A);
      ref Vertex v1 = ref Unsafe.Add (ref vBase, seg.B);
      // InsertVertex grows mT/mN only within the pre-grown capacity; mV is never grown here
   ...
   ref Tile Locate (Point2 pt) {
      ...
      ref Segment sBase = ref MemoryMarshal.GetArrayDataReference (mS);
      ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
      ref Node node = ref nBase, prev = ref node;
      for (; ; ) {
         bool first;
         switch (node.Kind) {
            case ENode.Y: first = pt.Y < Unsafe.Add (ref vBase, node.Index).Pt.Y; break;
            case ENode.X: first = Unsafe.Add (ref sBase, node.Index).IsLeft (pt); break;
            case ENode.Leaf: return ref Unsafe.Add (ref tBase, node.Index);
            case ENode.Redirect:
         ...
         prev = ref node;
         node = ref Unsafe.Add (ref nBase, first ? node.First : node.Second);
      }
   }
   ...
   void MergeTiles (ref Tile t0, ref Tile t1) {
      ...
      if (t1.ETop != EChain.Mountain) {
         ref Vertex vBase = ref MemoryMarshal.GetArrayDataReference (mV);
         ref Vertex vt = ref Unsafe.Add (ref vBase, t0.VTop);
      ...
      ref Node nBase = ref MemoryMarshal.GetArrayDataReference (mN);
      ref Node n0 = ref Unsafe.Add (ref nBase, t0.Node);
   ...
   int SliceTiles (ref Segment seg) {
      ...
      // tBase is valid for the entire loop: Grow above ensures no reallocation inside it
      ref Tile tBase = ref MemoryMarshal.GetArrayDataReference (mT);
      ...
      for (int i = 0; i < n; i++) {
         ref Tile left = ref Unsafe.Add (ref tBase, mLefts[i]);
         ref Tile right = ref left.SplitX (this, seg.Id);
         mRights.Add (right.Id);
         if (left.VTop == 0) MergeTiles (ref Unsafe.Add (ref tBase, mLefts[i - 1]), ref left);
         if (right.VTop == 0) MergeTiles (ref Unsafe.Add (ref tBase, mRights[i - 1]), ref right);
      }
      return mRights[^1];
   }
