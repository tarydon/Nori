...
public class MeshSlicerExp {
   // Constructor --------------------------------------------------------------
   public MeshSlicerExp (ImmutableArray<Mesh3> meshes) => mMeshes = meshes;
   ...
   int GetNode (int a, int b, double da, double db) {
      ...
      // packed into it)
      uint key = (uint)(a < b ? ((uint)a << 16) + b : ((uint)b << 16) + a);
...
public class MeshSlicerFinal {
   // Constructor --------------------------------------------------------------
   public MeshSlicerFinal (ImmutableArray<Mesh3> meshes) => mMeshes = meshes;
   ...
   int GetNode (int a, int b, double da, double db) {
      ...
      if (mBigMesh) {
         ...
         uint key = (uint)(a < b ? ((uint)a << 16) + b : ((uint)b << 16) + a);
         if (mSmallEdgeMap.TryGetValue (key, out idx)) return idx;
         mSmallEdgeMap.Add (key, mUsedNodes);
      }
   ...
   bool PrepMesh (Mesh3 mesh) {
      ...
      // Reset some intermediate data structures we're going to use
      mBigEdgeMap.Clear (); mSmallEdgeMap.Clear ();
      mBigMesh = n >= 65535;
      return true;
   }
   bool mBigMesh;
